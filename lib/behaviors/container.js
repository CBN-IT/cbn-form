	...
	CbnForm.FormContainer = {
		...
		_registerFormElement: function(event) {
			...
			if (element._hasModelValue) {
				...
				this._initializeModelPath(element.name);
			}
		...
		_formModelChanged: function(changes) {
			...
			if (components.length == 1) { // model changed
				...
				elements = this._modelPathPrefixMap[prefix];
			}
		...
		/**
		 * Creates all model object entries along the path.
		 * 
		 * @param path The path to initialize (without the 'model.' prefix).
		 * @protected
		 */
		_initializeModelPath: function (path) {
