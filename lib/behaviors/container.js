(function(CbnForm) {
	...
	CbnForm.FormContainer = {
		...
		/**
		 * Searches for the form element with the given name.
		 * If multiple elements use the same name, the behavior is undefined (either one of them can be returned).
		 * 
		...
		getElementByName: function (name) {
			for (var i=0; i<this.formElements.length; i++) {
				var el = /** @type {FormElement} */this.formElements[i];
				if (el.name == name) {
		...
		validate: function(noFocus) {
			...
			this.formElements.forEach(function(el){
				var input = /** @type ValidatableInput */(el);
		 ...
		 * ```
		 *     form.reset('personal'); // will reset only the inputs with the path prefix 'personal.'
		 * ```
		 * 
		...
		reset: function(path, value) {
			...
			this.set((path ? 'model.' + path : 'model' ), value);
		...
		_registerFormElement: function(event) {
			...
			if (element._hasModelValue) {
				...
				this._initializeModelValue(element.name, element.defaultValue);
			}
		...
		_formModelChanged: function(changes) {
			...
			if (elements) {
				...
				elements.forEach(function (input) {
					this._initializeModelValue(input.name, input.defaultValue);
				}, this);
		...
		/**
		 * Creates all model object entries along the path and initializes it with the specified default value.
		 * 
		 * @param {String} path The path to initialize (without the 'model.' prefix).
		 * @param {*} defaultValue The default value to set (if the current value is undefined). 
		 * @protected
		 */
		_initializeModelValue: function (path, defaultValue) {
			...
			if (!value && value !== false) // unchecked checkbox, maybe?
