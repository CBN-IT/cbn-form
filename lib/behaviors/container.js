(function(CbnForm) {
	 ...
	 * 
	 * - maintains a list of contained form elements: `formElements`;
	 * - it hides the contained elements so that the it can be used to create a standalone element from multiple using 
	 *   elements as children.
	 * - if the `model` property is non-null, binds the inputs to a user-supplied model object which (two-way: 
	 *   when the input is changed, the model changes and vice-versa);
	 * - automatically detects when the model is changed and sets the `isDirty` attribute; the dirty flag is 
	 *   removed when the model reverts to its initial values (for which you can use `reset`).
	 * 
	 * In order for the model binding to work, all model changes must be done using Polymer's property notification 
	 * system (aka use the `Polymer.Base.set` method):
	 * ```javascript
	...
	CbnForm.FormContainer = {
		...
		reset: function(path, value) {
			...
			try {
				this._modelChangeEffects = function (input) {
					...
					this._setElementDirtyStatus(input, false);
				}.bind(this);
		...
		/**
		 * Returns whether the specified path is dirty.
		 * 
		 * @param {String} name The path to check.
		 * @return {Boolean} True if dirty.
		 */
		isPathDirty: function(name) {
			return (this._dirtyModelPaths.indexOf(name) >= 0);
		},
		...
		_registerFormElement: function(event) {
			...
			if (this._inputHasModelValue(element)) {
				...
				this._setElementDirtyStatus(element, false);
				if (this.model) {
					var oldEffects = this._modelChangeEffects;
					try {
		...
		_unregisterFormElement: function (event) {
			...
			// remove it from the dirty map, if present
			this._setElementDirtyStatus(element, false);
		...
		_formElementValueChanged: function(event) {
			...
			try {
				...
				// only set dirty on direct value changes
				if (!event.detail || !event.detail.indirect) {
					this._setElementDirtyStatus(element, true);
				}
			} finally {
				this._modelChangeEffects = oldEffects;
			}
		 ...
		 * Also maintains the `_dirtyModelPaths` list and the `isDirty` flag.
		 * 
		...
		/**
		 * Sets the dirty flag of an element.
		 * 
		 * @param {CbnForm.InputElement} element The element to set the dirty flat on.
		 * @param {Boolean} dirty The new dirty status to set.
		 * @protected
		 */
		_setElementDirtyStatus: function(element, dirty) {
			var drtIdx = this._dirtyModelPaths.indexOf(element.name);
			if (drtIdx >= 0 && !dirty) {
				// we need to remove it
				this._dirtyModelPaths.splice(drtIdx, 1);
			} else if (drtIdx == -1 && dirty) {
				// we need to add it
