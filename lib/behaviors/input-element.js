(function(CbnForm) {
	 ...
	 * 
	 * - the `value` property provides read/write access to the input's value;
	...
	CbnForm.InputElementImpl = {
		...
		properties: {
			...
			value: {
				type: String,
				value: '',
				notify: false, /** event is manually fired, see `_fe_valueChanged` */
		...
		/**
		 * Will automatically be set to true if the `value` property is indirectly changed (i.e. from the form's 
		 * model).
		 * 
		 ...
		 * 
		 * If you want to customize / avoid firing this event while changing the input's value, see `_setIndirectValue`.
		 * 
		 * @event value-changed
		...
		/**
		 * Observer for the form element's `value` property.
		 * 
		 * It manually fires the 'value-changed' event conditionally if `_inputValueIndirectlyChanged` is false.
		 * 
		 ...
		 * 
		 * Should fire notification events (if `_inputValueIndirectlyChanged` is not true).
		 * 
