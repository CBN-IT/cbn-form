(function(CbnForm) {
	 ...
	 * See the `_setInternalValidationState` API method for usage details.
	 * 
	 * @polymerBehavior
	 */
	CbnForm.Validatable = {
		...
		properties: {
			 ...
			 * settings parameter to pass to the validator (see `CbnForm.Validator` for a list).
			 */
			validation: {
				type: Object,
		 ...
		 * @param {Boolean} [dontNotify] Set to true to prevent firing the validation event.
		 * @return {Boolean} The input's resulting validation state (i.e. true if valid).
		 */
		_setInternalValidationState: function(valid, dontNotify) {
