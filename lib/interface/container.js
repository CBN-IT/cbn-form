	...
	CbnForm.AbstractContainer = {
		...
		ready: function() {
			...
			this.addEventListener('cbn-form-control-detached', function (event) {
				...
				// also notify the model container, if different from current container
				if (element.modelContainer && element.modelContainer != container) {
					container.fire('cbn-form-control-detached', {
						element: element
					}, element.modelContainer, true, false);
				}
				
				var idx = container.childControls.indexOf(element);
				if (idx >= 0) container.childControls.splice(idx, 1);
				
				if ((element.hasModelValue || element.isFileInput) && element.modelContainer == container) {
